...
public class InstanceOf extends AbstractExpr {
    ...
    protected GPRegister codeGenReg(DecacCompiler compiler) {
        ...
        /*compiler.addInstruction(new LEA(type.getClassDefinition().getvTable().getOperand(), getR(1)));
        ...
        LOG.info("appel fonction qui code la comparaison des classes et stocke l'info dans R0");
        codeGenInstanceOf(compiler);

        compiler.addInstruction(new LOAD(getR(0), res)); //on récupère le résultat stocké dans R0
        */
        return res;
    ...
    protected void codeGenInstanceOf(DecacCompiler compiler){
        ...
        Label compareClass = compiler.getLabelManager().newLabel("instanceOf_Compare_Class");

        compiler.addInstruction(new BEQ(notInstanceOf)); //si objet est nul c'est pas instanceOf d'une class

        compiler.addLabel(compareClass); //comparaison des classes
        compiler.addInstruction(new CMP(getR(1), getR(0)));
        compiler.addInstruction(new BEQ(isInstanceOf)); //si égal c'est instanceOf de l'autre
        ...
        compiler.addInstruction(new LOAD(0, getR(0))); //on mets 0 dans R0 car pas instanceOf
        compiler.addInstruction(new RTS());

        compiler.addLabel(isInstanceOf);
        compiler.addInstruction(new LOAD(1, getR(0))); //on mets 1 dans R0 car instanceOf de l'autre
        compiler.addInstruction(new RTS());
        */
    }
}
